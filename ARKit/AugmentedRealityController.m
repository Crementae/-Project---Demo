...
- (void)setCenterLocation:(CLLocation *)newLocation
{
	...
	for (ARGeoCoordinate *geoLocation in [self coordinates]) {
		...
		if ([geoLocation isKindOfClass:[ARGeoCoordinate class]]) {
            ...
            //NSLog(@"setCenterLocation: %f %f", [geoLocation radialDistance], _radarRange);
            
            //if(([geoLocation radialDistance]) > _radarRange){
              //  continue;
            //}
...
- (void)removeAllCoordinate{
    for (ARGeoCoordinate *item in [self coordinates]) {
        ARMarker *markerView = [item displayView];
...
- (BOOL)shouldDisplayCoordinate:(ARCoordinate *)coordinate
{
    ...
    ARGeoCoordinate *geoCoordinate = coordinate;
	
    //NSLog(@"Current %f, Item %f, delta %f, range %f",currentAzimuth,pointAzimuth,deltaAzimuth,degreesToRadian(degreeRange));
    
	if (deltaAzimuth <= degreesToRadian(degreeRange))
		result = YES;
    
    CLLocationDistance distance = [centerLocation distanceFromLocation:[geoCoordinate geoLocation]];
    
    //NSLog(@"Location: %f, %f",geoCoordinate.geoLocation.coordinate.latitude,geoCoordinate.geoLocation.coordinate.longitude);
    //NSLog(@"Radial: %f, %f",distance, _radarRange);
...
- (void)updateLocations
{
    ...
    //NSLog(@"viewAngle: %f",viewAngle);
	
    NSMutableArray *markerAdded = [NSMutableArray array];
	...
	for (ARGeoCoordinate *item in [self coordinates]) {
        ...
        // Check marker in range and accelerater in z-aix (30%) <= 0.7 & >= -0.7
        if ([self shouldDisplayCoordinate:item]
            && (lastAcceleration != nil && lastAcceleration.z < 0.7 && lastAcceleration.z > -0.7)) {
            ...
            double markerAlt = markerView.coordinateInfo.geoLocation.altitude;
            double centerAlt = centerLocation.altitude;
            
            // convert ft to m
            //if(markerAlt > 1000){
            //    markerAlt = markerAlt / 3.2808;
            //}
            
            //NSLog(@"%f, %f",centerAlt, markerAlt);
            double deltaAlt = (centerAlt - markerAlt);
            // easy for altitude...
            transform = CATransform3DTranslate(transform, 0, deltaAlt, 0);
            //transform = CATransform3DRotate(transform, [self maximumRotationAngle] * lastAcceleration.y / 90, 0, 0, 0);
            
            //NSLog(@"%f",lastAcceleration.y);
			...
			if ([self rotateViewsBasedOnPerspective]) {
                ...
                //NSLog(@"%f,%f",centerAzimuth,itemAzimuth);
				...
				//transform				= CATransform3DRotate(transform, (lastAcceleration.y * 100) * M_PI / 90 , 0, 1, 0);
			}
			[[markerView layer] setTransform:transform];
            //markerView.transform = CGAffineTransformMakeRotation(degreesToRadian(90));
			
			//if marker is not already set then insert it
			if (!([markerView superview])) {
                [markerAdded addObject:markerView];
        ...
        // Update coordinates
        NSMutableArray *sortedArray = [markerAdded sortedArrayUsingComparator:^NSComparisonResult(id obj1, id obj2) {
            ARMarker *m1 = (ARMarker *)obj1;
            ARMarker *m2 = (ARMarker *)obj2;
            double first = [m1.coordinateInfo distanceFromOrigin];
            double second = [m2.coordinateInfo distanceFromOrigin];
        ...
        for(ARMarker *markerView in sortedArray){
            [[self displayView] insertSubview:markerView atIndex:1];
        }
        
        //[coordinates setArray:sortedArray];
        //coordinates = sortedArray
	}
}
